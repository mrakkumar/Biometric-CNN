...
Class Run():
	...
	def img_count(self):
		self.imgcount = len(list(data_dir.glob('*/*.{}'.format(self.filetype))))
	...
	def get_label(self, file_path):
  	...
  	def make_pair(self):
		...
		self.train = self.train.map(process_path, num_parallel_calls=AUTOTUNE)
		self.vals = self.vals.map(process_path, num_parallel_calls=AUTOTUNE)

	def get_pairs(self, dataset):
		# Use if fitting with (data, label) input
		return image_batch, label_batch = next(iter(dataset))
	...
	def configure_for_performance(self, ds):
  	...
  	def performance(self):
  		self.train = configure_for_performance(self.train)
		self.vals = configure_for_performance(self.vals)
	...
	def get_model(self):
		...
		ipshape = self.train.take(1)[0].np().shape
		## GENERALISE THIS
		mc = ModelClass()
		mc.make_model(ipshape, nclasses, seed=self.seed)
		self.Model = mc

	def mv_exists(self, foo):
        # If file/directory already exists 
        # renamed with suffix ".old" - creates type backup file
        try:
            os.rename(foo, foo+".old")
        except OSError:
            pass
